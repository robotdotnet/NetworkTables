...
namespace NetworkTables
{
    ...
    {
        private bool GetPersistentEntries(bool periodic, List<ValueTuple<string, Value>> entries)
        {
            using (m_monitor.Enter())
            {
                ...
                foreach (var i in m_entries)
                {
                    ...
                    entries.Add(new ValueTuple<string, Value>(i.Key, entry.Value));
                }
            }
            entries.Sort();
            return true;
        ...
        private static async Task SavePersistentImpl(StreamWriter stream, IEnumerable<ValueTuple<string, Value>> entries)
        {
            ...
            foreach (var i in entries)
            {
                ...
                await WriteStringAsync(stream, i.Item1).ConfigureAwait(false);
        ...
        {
            List<ValueTuple<string, Value>> entries = new List<ValueTuple<string, Value>>();
        ...
        public async Task<bool> LoadPersistentAsync(Stream stream, Action<int, string> warn)
        {
            ...
            using (StreamReader reader = new StreamReader(stream))
            {
                ...
                while ((lineStr = await reader.ReadLineAsync().ConfigureAwait(false)) != null)
                {
                    ...
                    {
                        entries.Add(new ValueTuple<string, Value>(name, value));
                    }
                ...
                try
                {
                    ...
                    {
                        Entry entry;
                        if (!m_entries.TryGetValue(i.Item1, out entry))
                        {
                            entry = new Entry(i.Item1);
                            m_entries.Add(i.Item1, entry);
                        }
                        ...
                        if (m_notifier.LocalNotifiers())
                        {
                            if (oldValue != null)
                            {
                                m_notifier.NotifyEntry(i.Item1, i.Item2, (NotifyFlags.NotifyNew | NotifyFlags.NotifyLocal));
                            }
                            else if (oldValue != i.Item2)
                            {
                                ...
                                m_notifier.NotifyEntry(i.Item1, i.Item2, notifyFlags);
                            }
                        }
                        ...
                        if (oldValue == null || oldValue.Type != i.Item2.Type)
                        {
                            msgs.Add(Message.EntryAssign(i.Item1, entry.Id, entry.SeqNum.Value, i.Item2, entry.Flags));
                        }
                        else if (entry.Id != 0xffff)
                        {
                            if (oldValue != i.Item2)
                            {
                                msgs.Add(Message.EntryUpdate(entry.Id, entry.SeqNum.Value, i.Item2));
                            }
                            if (!wasPersist)
